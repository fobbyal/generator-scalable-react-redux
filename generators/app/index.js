...
module.exports = Generator.extend({
  ...
  prompting() {
    ...
    this.log(yosay(`Welcome to the ${chalk.red('react-redux-spa')} generator!`))
    ...
    const prompts = [
      ...
      },
      {
        ...
        name: 'desc',
        message:
          'Please enter description for the app. This will be used as the Title for the webpage',
        default: this.appname,
      },
      {
        type: 'input',
  ...
  install() {
    this.installDependencies({
      ...
      callback: error => {
        if (error) {
          this.env.error(
            '… make sure yarn is installed properly ' +
    ...
    this.log('initializng git repo')
    this.spawnCommandSync('git', ['init'])
    this.log('setting up commit hook for prettier')
    this.spawnCommandSync('yarn', ['add', 'lint-staged', 'husky', '--dev'])
    this.log('running prettier')
    this.spawnCommandSync('yarn', ['pretty:write'])
    this.config.set('createdBy', this.rootGeneratorName())
    this.config.save()
    this.log(
      yosay(`
      That’s it. Feel free to fire up the server with ${chalk.green(
        'yarn start'
      )}
